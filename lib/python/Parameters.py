...
# 定义需要读取的文件夹路径
folder_path = r"/Users/zyqyq/Program/数据集/L2BP/202408"  # 请替换为你的文件夹路径
...
for filename1 in os.listdir(folder_path):
    ...
    if filename1 == '.DS_Store':
        continue
    folder_full_path = os.path.join(folder_path, filename1)
    # 检查是否为目录
    if os.path.isdir(folder_full_path):
        for filename2 in os.listdir(folder_full_path):
            # 跳过 .DS_Store 文件
            if filename2 == '.DS_Store':
                continue
            if filename2.endswith('.TXT'):
                file_path = os.path.join(folder_full_path, filename2)
                date_time_str = filename2.split('_')[-4].split('.')[0]
                dt = datetime.strptime(date_time_str, '%Y%m%d%H%M%S')

                dt_str = dt.strftime('%Y-%m-%d %H:%M:%S')

                MSTstr = filename2.split('_')[-2].split('.')[0]
                print(date_time_str,MSTstr)
                if MSTstr == 'M':
                    MST = 0
                else:
                    MST = 1
