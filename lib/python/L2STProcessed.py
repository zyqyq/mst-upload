...
# 定义需要读取的文件夹路径
folder_path = "C:\\Users\\HUAWEI\\Desktop\\L2" # 请替换为你的文件夹路径
...
for filename1 in os.listdir(folder_path):
    for filename2 in os.listdir(os.path.join(folder_path, filename1)):
        if filename2.endswith('ST_processed.TXT'):   
            ...
            with open(file_path, 'r') as file:
                ...
                if len(lines) >= 23:  # 文件至少有34行
                    ...
                    for line in lines:
                        ...
                        if math.isnan(horiz_ws):
                            horiz_ws = -99999999
                        if math.isnan(horiz_wd):
                            horiz_wd = -99999999
                        if math.isnan(verti_v):
                            verti_v = -9999999
                        if math.isnan(Cn2):
                            Cn2 = -9999999
                        if math.isnan(Credi):
                        ...
                        # 执行 SQL 语句
                        sql = "INSERT INTO smos_radar_qzgcz_L2STProcessed (Time,show_name,name,Platform_id, Height, Horiz_WS, Horiz_WD, Verti_V, Cn2, Credi) VALUES (%s,%s, %s,%s,%s, %s, %s, %s, %s, %s)"
